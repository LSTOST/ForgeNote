...
## 下一步收口
M1 计划票 I-08~I-23 全部 Done；DSN-01 已 Done；PR #10 / PR #11 / PR #12 / PR #13 / PR #14 / PR #15 / PR #16 / PR #17 / PR #18 / PR #19 / PR #20 / PR #21 / PR #22 已合并。PR #24 只做 DSN-02 边界裁决，不改变当前唯一任务。当前唯一任务仍是 V-01 Production 真实非构建者用户主路径验证；DSN-02 实现排在 V-01 收口之后。不要把资产库、自动学习、内容日历塞进下一步。
...
## 最后更新时间
2026-06-30 (PR #24 / DSN-02 Codex 裁决：DSN-02 承接 V-01-FIX-09，不取代 V-01，也不允许并行打架。FIX-09 只能收口登录/注册模式辨识与登录表面冻结；若当前 FIX-09 删除 Magic Link，则必须在合入前恢复迁移安全，或保持 Draft/关闭。D-10 已记录：长期登录方式收敛为邮箱密码 + Google；密码重置迁移路径必须先于 Magic Link 下线可用；`/auth/callback` redirect allowlist 需覆盖 Production / Preview wildcard / localhost；登录失败与重置请求不得泄露账号是否存在；“记住 30 天”不得直接做假复选框，需先拆 AUTH-SPIKE-30D 验证 Supabase SSR/session policy 可行性。排期结论：V-01 先行，DSN-02 实现排在至少 1 个真实非构建者用户证据之后。)
