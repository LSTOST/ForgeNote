...
## 当前票
当前唯一进行中票：**I-22 In Progress**。PR #9 已 squash merge 到 `main`（`77e5b80`），DSN-01、I-20、I-21 均已 Done。I-22 在独立分支 `codex/i-22-content-plan-usability` 实现，目标是把第一份生成结果从松散 AI 输出升级为可复制、可发布前检查、值得判断是否保存的结构化初稿。

## 当前分支
当前工作分支：`codex/i-22-content-plan-usability`。分支基于 `main` / `origin/main` = `77e5b80`（PR #9 合并后基线）。

## 当前 PR
PR #9：`https://github.com/LSTOST/ForgeNote/pull/9` 已 squash merge。当前 I-22 尚未开 PR。
...
## 进行中
- **I-22（In Progress / Gate 3 blocked on Supabase redirect）**：已从 `main` 开独立分支并开 Draft PR #10；实现中包含生成契约/prompt 最小升级、逐页卡片文案与配图方向、发布前检查、保存配方前价值判断。已通过 `doctor` / `lint` / `typecheck` / `build` / `smoke:api`，PR #10 CI/Vercel 绿。Preview alias 已确认可达；Owner 在精确 Preview 登录后被 OAuth 送回 Production，根因为 Supabase Auth Redirect URLs 未放行 PR #10 Preview callback。
...
## 下一步收口
M1 计划票 I-08~I-21 全部 Done；DSN-01 已 Done；PR #9 已合并。当前收口 I-22：第一份内容方案可用性升级。不要把视觉渲染、资产库、自动学习塞进 I-22。观测真实 SDK / runtime i18n 不默认进入；当 Production 出现外部真实用户后，应回填 `docs/acceptance/I-19.md` 的 Gate 3 表并复核 Gate 4 指标。
