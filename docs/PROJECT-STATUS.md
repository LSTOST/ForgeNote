...
## 当前 PR
Draft PR #9：`https://github.com/LSTOST/ForgeNote/pull/9`。状态：open draft，merge state clean；GitHub Actions `Doctor / Lint / Typecheck / Build` 通过，Vercel Preview Ready；PR body 已补合并证据。Supabase Auth Redirect URL 修正后，PR Preview 登录态已打通：`/forge` 显示 `dennisliu1225@gmail.com`。Preview Gate 3 已跑到生成阶段：输入 → 方向确认 → 编辑受众为 `新手父母` → `已确认 1/3` → 最终生成。结果被 Preview runtime `OpenRouter 返回非 2xx 状态：401` 阻断；I-21 草稿 session URL 行为通过，URL 变为 `/forge?session=46470ef0-eb43-459d-8d19-f060d35d0cd7`，刷新后恢复输入、假设和错误态。PR #9 暂不转 Ready，剩余阻塞为 Vercel Preview `OPENROUTER_API_KEY` 无效/缺失。
...
## 已通过验收
- I-21：自动验证 `lint` / `typecheck` / `build` 通过；Preview 登录态实测：`/forge` 渲染登录用户 `dennisliu1225@gmail.com`，输入 → 方向确认 → 编辑「受众」为「新手父母」→ `已确认 1/3`；生成因 Preview OpenRouter `401` 失败，但草稿 session URL 写入 `/forge?session=46470ef0-eb43-459d-8d19-f060d35d0cd7`，刷新后恢复输入、假设和错误态。见 `docs/acceptance/I-21.md`。I-21 草稿刷新通过；完整成功生成仍被 Preview runtime key 阻塞，PR #9 保持 Draft。
