...
## 当前票
I-11 — 偏好页 `/profile`：edited assumptions 写入并下次带出（Done）
...
**边界（review / 验收须守住）**
- I-15 / I-16 已按 additive 路线完成；当前唯一任务是 **I-11**。
...
## 进行中
- M1 主线票（I-08~I-11、I-15、I-16）已交付；剩余 Backlog：I-12（F-16）、I-13（eval 门禁）、I-14（观测）、I-17（i18n 资源外化）

## 已通过验收
- I-11：偏好页 `/profile` CRUD + `/forge` 偏好带出 + edited assumptions「记住为偏好」通过自动验证与匿名边界；真实 Chrome 登录态验收见 docs/acceptance/I-11.md
...
## 下一张唯一任务
I-12 — F-16 表现回填 lite（`POST /api/sessions/:id/performance`）（不做 perf_score / 不做自动抓取）
...
## 最后更新时间
2026-06-21 (I-11 偏好页 /profile 完成：profile_preferences CRUD API（GET/POST upsert/PUT/DELETE）+ 受保护 /profile 页 + /forge 按 RLS 带出 source=profile 偏好假设 + AssumptionPanel「记住为偏好」写入；无 migration（表/RLS 已存在）；back-compat 无偏好不变；自动验证与匿名边界通过；下一张唯一任务 I-12)
