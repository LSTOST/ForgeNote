...
## 当前 PR
Draft PR #9：`https://github.com/LSTOST/ForgeNote/pull/9`。状态：open draft，merge state clean；GitHub Actions `Doctor / Lint / Typecheck / Build` 通过，Vercel Preview Ready；PR body 已补合并证据。Preview `/forge` hosted Gate 3 复测尝试被 Chrome 登录安全策略挡住：访问 `/forge` 会转 `/login`，Google 登录动作被拦截；未绕过。Botmail fallback 已创建 `forgenote-pr9@thebrownpig.org` 并成功收到 Supabase 确认邮件，但 Botmail 仅暴露纯文本正文，未暴露 HTML 按钮 URL；本地也无 `SUPABASE_SERVICE_ROLE_KEY` 可生成 admin Magic Link。Owner 反馈登录入口无法打开后，已确认 `/login` HTTP 200、`/forge` 307→`/login`，Vercel share URL 也仍被应用登录拦截；Owner 实测 Google OAuth 最终落到 `localhost` / `ERR_CONNECTION_REFUSED`。代码使用 `window.location.origin/auth/callback`，无 localhost 硬编码；当前根因收敛为 Supabase Auth Redirect URL 未放行 PR Preview callback，回退到 Site URL。PR #9 暂不转 Ready。
...
## 已通过验收
- I-21：自动验证 `lint` / `typecheck` / `build` 通过；代码路径核对通过。`ForgeWorkbench` 在成功生成与草稿失败时更新 `/forge?session=`，刷新复用既有 Server Component 预载 session 逻辑；点击「新建」或重新进入方向确认清理旧 session query；Preview hosted 复测尝试被 Chrome 登录安全策略挡住，Botmail fallback 又卡在 HTML Magic Link URL 不可见，未绕过；见 `docs/acceptance/I-21.md`。状态 Done，但 PR #9 仍保持 Draft。
