...
## 当前票
I-16 — `output_locale` 数据与生成链路支持（Done，DB 写入手测 Blocked 待应用 migration 0002）
...
- 登录态验收（部分 Blocked）：真实 Chrome 登录态下新输入控件渲染正常；**无 locale 生成 200 成功（既有流程未回归）**；带 locale 生成因 migration 0002 未应用到共享 DB 返回 `DATABASE_ERROR`（错误态优雅，不白屏）。直接对生产库应用 migration 被安全策略拦截，未绕过 → locale 写入持久化 / `GET /api/sessions` 回 outputLocale / rerun 带 locale 标 **Blocked**，待 Owner 应用 0002 后复测（见 docs/acceptance/I-16.md）

## 进行中
- `/profile` 偏好页尚未交付（I-11，下一张唯一任务）
- I-16 migration `0002_output_locale.sql` 待 Owner 应用到 Supabase（应用后 locale 写入链路即生效）

## 已通过验收
- I-16：output_locale 数据与生成链路通过自动验证与匿名边界；登录态无 locale 生成未回归。locale 写入持久化手测 Blocked（待应用 migration 0002），进入 Done（代码+迁移已交付）
...
- **I-16 migration `0002_output_locale.sql` 待 Owner 应用到 Supabase**：自动化环境直接对生产库执行被安全策略拦截（未绕过）。应用前带 locale 的生成/重跑会返回 `DATABASE_ERROR`（无 locale 流程不受影响）；应用后 locale 写入与 `GET /api/sessions` 回传即生效。

## 下一张唯一任务
I-11 — 偏好页 `/profile`：edited assumptions 写入并下次带出（不做自动学习 / 不做 I-12 F-16）
...
## 最后更新时间
2026-06-21 (I-16 output_locale 完成：migration 0002 给 sessions 增 nullable text 列、forge/rerun/session API 串通、生成链路最小约束、UI 自由文本输入；additive 不 default/enum/backfill、不改 RLS、不重写 prompt；无 locale 流程未回归；locale 写入手测 Blocked 待 Owner 应用 0002；下一张唯一任务 I-11)
