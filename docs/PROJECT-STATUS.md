...
- **V-01-FIX-08（Done）**：PR #22 已 squash merge 到 `main`（`7b78558`）。`/login` 已对齐 Claude Design：纸感暖底 + 点阵纹理、38px 暖色 Zap 闪电标、`ForgeNote` 独立 serif 品牌标题、`图文卡片内容工作台` 小号分类线、slogan、深橙 `#B5562B` 主按钮、`#FFFDF9` 输入背景、`#E3D8C7` warm border。未改 Supabase、`/auth/callback`、业务 API、DB、RLS、prompt、Forge 工作台或登录能力。Preview Gate 3 与 Production recheck 均通过。
...
## 最后更新时间
2026-07-01 (V-01-FIX-09 视觉保真二次补丁：按 `ui-ux-pro-max` 表单/响应式规则与 `/Users/tete/Downloads/DSN-02 Login Auth.dc.html`，PR #26 `/login` 从 `lg:grid-cols-2` 50/50 改为左侧固定 `w-[524px]` + 右侧弹性表单区，表单仍 max 344px；左侧四块角色放大为 620x520 主视觉，状态页/移动端不再使用单个橙色脸，统一用缩小版角色组，避免创建账号/重置状态图形撑爆；`保持 30 天登录` 改为真实 checkbox 控件。未改 Supabase、`/auth/callback`、业务 API、DB、RLS、prompt、Forge 工作台或 Magic Link 边界。`doctor` / `lint` / `typecheck` / `build`（联网重跑）/ local `smoke:api` / `/login` HTML 结构检查 / `git diff --check` 通过；待推送后重跑 PR #26 CI/Vercel/Preview Gate。)
2026-07-01 (Owner 发现 PR #26 创建账号后 QQ 邮箱收不到确认邮件。Codex 判定为 V-01-FIX-09 邮件送达恢复缺口：代码不能把 `signUp()` 成功等同于邮件送达。已补 `/login` signup sent 状态的「重新发送确认邮件」，调用 `supabase.auth.resend({ type: "signup", email, options.emailRedirectTo })`，并增加重发 loading/success/error；文案改为“确认邮件请求已发送”，提示检查垃圾箱/拦截箱、QQ/企业邮箱可能拦截，仍收不到则换邮箱或用 Google。`docs/DEPLOYMENT.md` 明确 Production/Preview 不应长期依赖 Supabase 默认邮件，真实用户前需配置可靠自定义 SMTP，并用 QQ + 非 QQ 邮箱实测注册确认和密码重置。`doctor` / `lint` / `typecheck` / `build` / Preview `smoke:api` / `git diff --check` 通过；PR #26 已推送，GitHub Actions 与 Vercel 均 PASS。)
2026-07-01 (V-01-FIX-09 Post-patch Preview Gate 3 Pass：PR #26 head `5996064`，GitHub Actions Doctor/Lint/Typecheck/Build PASS，Vercel PASS，Preview `smoke:api` PASS。Preview `/login` HTML 确认满屏 `lg:grid-cols-2`、plain ForgeNote wordmark、四块角色色块、30 天登录勾选方块、欢迎回来/邮箱/密码/忘记密码/Google/创建账号存在且 Magic/发送登录链接/登录链接 absent；`/login/reset` 可渲染设置新密码状态。Chrome 退出该 Preview 登录态后视觉确认：满屏左右分栏、左侧四块角色未被中线裁切、右侧邮箱密码主路径 + Google 备选、30 天登录为勾选方块。V-01-FIX-09 UI/auth entry redesign 可判 Pass；剩余可选证据是已确认邮箱密码账号登录与重置邮件投递。)
2026-06-30 (V-01-FIX-09 Owner 视觉反馈补丁：PR #26 本地分支已把 `/login` 桌面从居中卡片改为满屏 50/50 split view；左侧改为纯 `ForgeNote` serif wordmark + 目标风格四块角色，移除火焰 icon；`保持 30 天登录` 改为已勾选方块；主按钮不再因空输入呈灰态，空输入提交显示 inline validation；注册成功 / 重置请求 / 重置邮件页 mascot wrapper 补 `relative`，修复创建账号确认页橙色图形撑爆全屏。`doctor` / `lint` / `typecheck` / `build` / local `smoke:api` / `git diff --check` 通过；本地 Chrome 视觉确认 split view、四块角色未裁切、无 Magic Link 文案。待推送后重跑 PR #26 Preview Gate。)
2026-06-30 (V-01-FIX-09 Preview Gate 3 Conditional Pass：PR #26 Draft，GitHub Actions PASS，Vercel PASS，Preview `smoke:api` PASS。Preview `/login` HTML 确认有欢迎回来/保持 30 天登录/忘记密码/Google/创建账号，且无 Magic/发送登录链接/登录链接；`/login/reset` 确认 ForgeNote/设置新密码/正在确认重置链接可渲染。Playwright 截图在当前 macOS sandbox 被 Chromium MachPort 权限阻止，未计入视觉证据。剩余：Owner/浏览器视觉确认；如要转 Ready，最好补一个已确认邮箱账号的密码登录与重置邮件路径。)
...
2026-06-29 (V-01-FIX-07 Preview Gate 3 Pass：PR #21 head `53b2ed9`，GitHub CI / Vercel 绿，merge state CLEAN。Preview `https://forge-note-git-codex-v-01-fix-07-login-7f2363-lstosts-projects.vercel.app/login` 匿名 desktop/mobile 视觉检查通过：1280x720 下登录模块 380px、390x700 下 350px 且左右 20px，无横向溢出；Codex QA 曾发现移动端标题孤立 `台`，Claude Code 以 `break-keep` 修正为 `ForgeNote` / `图文卡片内容工作台`。Preview `smoke:api` 通过；旧 footer 文案未回归。结论：可合并；合入后恢复 V-01 真实用户验证。)
2026-06-29 (Owner 指出当前 `/login` 登录框左右拉得太长、没有设计美学。Claude Design 已交付 `/Users/tete/Downloads/Login B Final.dc.html`；Codex 审查后切 V-01-FIX-07：只采纳视觉比例和状态规格，不直接照搬原型 HTML/CSS；不引入新字体/独立 logo/整页米色品牌系统；不改 Supabase、`/auth/callback`、业务 API、DB、RLS、prompt、Forge 工作台。)
