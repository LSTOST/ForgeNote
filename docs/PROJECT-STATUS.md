...
## 进行中
- **I-22（In Progress / Gate 3 blocked on manual login）**：已从 `main` 开独立分支并开 Draft PR #10；实现中包含生成契约/prompt 最小升级、逐页卡片文案与配图方向、发布前检查、保存配方前价值判断。已通过 `doctor` / `lint` / `typecheck` / `build` / `smoke:api`，PR #10 CI/Vercel 绿。Preview alias 已确认可达；剩余登录态 Gate 3，需要 Owner 先手动登录 Preview。
