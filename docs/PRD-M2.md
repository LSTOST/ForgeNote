...
> 版本：M2 / v1.1  | 日期：2026-07-11  | 状态：**Frozen｜冻结的实现基线**
> 本文不再接收新产品需求。当前产品需求事实源为 `PRD-CCOS.md`；本文仅用于解释现有 M2 代码、工程概念、Ticket 与验收材料。
> 在 CCOS Roadmap、工程契约和迁移矩阵完成前，本文保留在当前目录，不归档；前身 M1 已归档至 `docs/archive/m1/`。
