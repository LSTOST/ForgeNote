# ForgeNote 重定义 v3.16 · 冻结送审包（To: Codex）

> 2026-07-03 冻结（v3.15 后又按 Owner 反馈做暖化配色 + 点阵纸，升级为 v3.16——纯视觉修订，产品定义不变）。
> Owner 已确认 UI 形态与产品定义，本包为技术负责人 review 的输入。
...
| 产物 | 路径 | 状态 |
|---|---|---|
| 方向文档 v3.16 | `docs/ForgeNote_重定义方向_v3.md` | 冻结（含 v3.0→v3.16 完整变更日志） |
| **M1 PRD（含商业化）** | `docs/ForgeNote_M1_产品说明与实现方案_含商业化.md` | 冻结（Owner 撰写，已按五裁决与方向文档对齐） |
| Design Tokens | `docs/design/redefine-2026-07/design-tokens.md` | Owner 定稿 v3.16 暖化（暗色变体待确认） |
| 工作区概念稿 | `docs/design/redefine-2026-07/concept.html`（存档 `archive/concept-v3.16.html`） | 冻结 |
...
- ~~Recipe 沉淀为后台机制~~ **v3.15 已裁决**：配方是用户可见的显式资产
  （底栏"保存为配方"+ 结构模板库），但只存生产方法、禁存正文（PRD §6.6）。
- **内容原型层已定**（M1 五类，PRD §3）——与评审问题 1（token 词表治理）直接相关：
  原型 = 顶层封闭词表，槽位 token 词表挂在原型之下，请以此为前提回答问题 1。
- M1 范围：结构做 Narrative + Visual 两类；renderer 做 小红书 / X / 图片 Prompt 三个；
  Temporal 与视频类 renderer 延后（方向文档 §4）。

## M2-00 裁决结果（2026-07-03，Owner 拍板，工程可开工）

Codex review（`CODEX-REVIEW.md`）已完成，Owner 就 3 处方向级异议拍板：

1. **雷达伪热度** → 删数字，改来源标签（账号匹配强/近期信号/历史有效/低证据）。
2. **产品重心 / Gate 0 焦点** → **选题闭环优先**（每周真实周更选题 + 完成发布闭环），不只优化"1 分钟出草稿"。
3. **平台位置** → 放宽：平台选择/导出控制只在底部渲染层；已发布/任务的平台标签（metadata）可显示。

自动采纳（Codex 推荐、无争议）：生成对象=renderer/Recipe（中区主稿始终可读）；
Recipe 只存 signal refs；M1 renderer 严格三个（小红书/X/图片 Prompt）。

概念稿已按上述改讫。M2 按 CODEX-REVIEW.md 票表推进，最短路径 M2-00→01→02→03→11 再做 UI。
...
**背景**：产品已完成重定义，v3.16 已冻结在分支 `docs/redefine-v3-freeze`（以分支最新提交为准；v3.16 仅为暖化配色 + 点阵纸的视觉修订，产品定义与 UI 结构同 v3.15，配色本身不在评审范围）。
入口文件：`docs/design/redefine-2026-07/REVIEW-HANDOFF.md`。
通读顺序：REVIEW-HANDOFF.md → `docs/ForgeNote_重定义方向_v3.md`（重点 §2/§4/§5/§7）→
`docs/ForgeNote_M1_产品说明与实现方案_含商业化.md`（PRD，已与方向文档对齐）→
