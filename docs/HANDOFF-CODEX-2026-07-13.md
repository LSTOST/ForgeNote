...
## 3. Gate 0 剩余功能票（真正的开发工作，按序）

| 票 | 内容 | 关键点 |
|---|---|---|
| G0S-02 | 账号大脑喂入「独居生活指南」品牌资料 | 用 `/account` 把品牌定位/栏目/叙事/禁忌喂进 `account_memory_items`，验证生成结果受约束 |
| G0S-03 | 手动选题入口 | 已基本可用（中区 textarea + `?idea=`），补顺手性，用真实 canvas 选题验 30 秒开工 |
| G0S-04 | 资产挂载最小版 | 成品挂手册/清单/网盘链接字段，导出带出。前后端都要加 |
| G0S-05 | 归档 + 表现回填接真实数据 | `/api/performance/fill` 已有，把回填入口从 /gate0 接到内容流 |
| G0S-06 | 公众号长文 renderer（切片 2） | 同一主内容派生长文，含手册长文介绍场景 + golden test |
| G0S-07 | 手册配套物料模板化（切片 3） | 商品详情页 + 长文介绍从手册生成 |
...
- **本地登录**：Supabase Auth 的 Redirect URLs **没有 localhost**，本地点 Google 登录会被甩到生产域名。要本地测需在 Supabase 后台 URL Configuration 加 `http://localhost:3100/**`，或用邮箱密码登录。
