...
| 票号 | 状态 | 目标 | 范围外 | 依赖 |
|---|---|---|---|---|
| I-23 | Review | 保存配方后的复用证据链：保存 I-22 结构化结果后，用户能直接进入配方详情并换输入重跑；新结果仍保留发布正文、逐页卡片、配图方向、发布前检查 | 资产库、视觉渲染、视觉配方、多账号、自动学习、内容日历、发布自动化、DB schema/RLS 改动 | 分支实现已完成；Gate 2 Pass；Preview Gate 3 待验证 |
...
下一步：
- 当前分支已完成保存成功后的“查看配方”入口与复用提示；Gate 2 通过，下一步在 PR Preview 上完成 Gate 3。
```
